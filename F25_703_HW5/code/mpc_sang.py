...
class MPC:
    ...
    def predict_next_state_gt(self, states, actions):
        ...
        for i in range(states.shape[0]):
            ...
            current_state = states[i]
            action = actions[i % actions.shape[0]]  # Handle broadcasting if needed

            # Reset environment and set to current state
            self.env.reset()

            # Set the environment state (use full state including goal if available)
            if len(current_state) >= 10:
                # If state includes goal coordinates
                self.env.set_state(current_state.tolist())
            else:
                # If state doesn't include goal, use current goal
            ...
            # Only keep the first 8 dimensions (exclude goal coordinates)
